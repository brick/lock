...
final class RemoteWorker
{
    ...
    public function createLock(string $lockName): RemoteLock
    {
        $this->sendCommand(new Command\CreateLock($lockName));
        $lockIndex = $this->receiveLockIndex();

        return new RemoteLock($this, $lockIndex);
    }

    /**
     * @param non-empty-list<string> $lockNames
     */
    public function createMultiLock(array $lockNames): RemoteLock
    {
        $this->sendCommand(new Command\CreateMultiLock($lockNames));
        $lockIndex = $this->receiveLockIndex();
    ...
    /**
     * Internal to RemoteWorker & RemoteLock.
     */
    ...
    /**
     * Receives the lock index from the worker after a createLock() or createMultiLock() command.
     * This method expects to receive the lock id within 1 second.
     */
    private function receiveLockIndex(): int
    {
        $timeoutSeconds = 1.0;
        $startTime = microtime(true);
        ...
        while (true) {
            ...
            if ($output !== '') {
                ...
                if (preg_match('/^CREATED:([0-9]+)$/', $output[0], $matches) !== 1) {
                    Assert::fail('Expected CREATED:<index>, but received: ' . $output[0]);
                }
        ...
        Assert::fail('Worker did not respond within 1s');
    }
